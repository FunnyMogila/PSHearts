    ...
    public static void startGame(int endScore)
    {
                ...
                && gamers.get(2).getScore() < endScore && gamers.get(3).getScore() < endScore)
        {
            ...
            for (int i = 0; i < 13; i++)    // Цикл одного раунда
            {
                ...
                System.out.println("Карты на руках у игроков:");
                for (Gamer gamer : gamers) {
                    for (int k = 0; k < gamer.getNumOfCards(); k++) {
                        System.out.print(gamer.getCards().get(k).getStringCard() + ",");
                    }
                    System.out.println();
                }
                System.out.println("Карты на столе:");
                for (Card card : cardsOnTable) {
                    System.out.println(card.getStringCard());
                }
                System.out.println("Очки игроков:");
                for (Gamer gamer : gamers) {
                    System.out.println(gamer.getScore());
                }
                System.out.println("Текущая масть: " + currentSuit.toString());
                System.out.println("Ход начал игрок под номером: " + (currMovePlayerID + 1));
                System.out.println();
